...
/// Strategy used when drawing individual data2 points over the sparkline.
enum PointsMode {
  /// Do not draw individual points.
  none,

  /// Draw all the points in the data2 set.
  all,

  /// Draw only the last point in the data2 set.
  last,
}

/// A widget that draws a sparkline chart.
///
/// Represents the given [data2] in a sparkline chart that spans the available
/// space.
///
...
/// at the provided data2 point. Their appearance is determined by the
...
class Sparkline extends StatelessWidget {
  /// Creates a widget that represents provided [data2] in a Sparkline chart.
  Sparkline({
    Key key,
    @required this.data2,
    this.lineWidth = 2.0,
  ...
  ///
  /// Each data2 entry represents an individual point on the chart, with a path
  ...
  /// Determines how individual data2 points should be drawn over the sparkline.
  ///
  /// Defaults to [PointsMode.none].
  final PointsMode pointsMode;

  /// The size to use when drawing individual data2 points over the sparkline.
  ///
  /// Defaults to 4.0.
  final double pointSize;

  /// The color used when drawing individual data2 points over the sparkline.
  ///
...
class _SparklinePainter extends CustomPainter {
  ...
  update() {
    if (max == null) {
      max = data2Points.reduce(math.max);
    }

    if (min == null) {
      min = data2Points.reduce(math.min);
    }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double widthNormalizer = width / data2Points.length;

    for (int i = 0; i < data2Points.length; i++) {
      double x = i * widthNormalizer + lineWidth / 2;
      double y =
          height - (data2Points[i] - min) * heightNormalizer + lineWidth / 2;

      if (pointsMode == PointsMode.all) {
        points.add(new Offset(x, y));
      }

      if (pointsMode == PointsMode.last && i == data2Points.length - 1) {
        points.add(new Offset(x, y));
      }
  ...
  bool shouldRepaint(_SparklinePainter old) {
    return data2Points != old.data2Points ||
